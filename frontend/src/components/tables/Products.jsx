...
export default function ProductsTable() {
  ...
  const getNextSortValue = (currentValue) => {
    const sortCycle = ["currentSellPrice", "stock", null];
    const currentIndex = sortCycle.indexOf(currentValue);
    return sortCycle[(currentIndex + 1) % sortCycle.length];
  };

  const getSortByText = (sortValue) => {
    switch (sortValue) {
      case "currentSellPrice":
        return "По цене";
      case "stock":
        return "По складу";
      default:
  ...
  const handleSortByToggle = () => {
    const newSortBy = getNextSortValue(localFilters.sortBy);
    handleLocalFilterChange("sortBy", newSortBy);
    updateSearchParams({ ...localFilters, sortBy: newSortBy });
  };

  const handleOrderToggle = () => {
    const newOrder = localFilters.order === "desc" ? "asc" : "desc";
    handleLocalFilterChange("order", newOrder);
    updateSearchParams({ ...localFilters, order: newOrder });
  };
    ...
    <div className="flex flex-col gap-4 min-h-[85vh]">
          ...
          <div className="center-row gap-2 w-full">
            <div className="center-row gap-1 w-full">
              <button
                ...
                className="btn-primary text-xs md:text-sm px-2 w-1/2"
              >
                {getSortByText(localFilters.sortBy)}
              </button>
              <button
      ...
      <div className="bg-support dark:bg-darkTwo border border-support-200 shadow shadow-support-200 rounded p-2 pb-4">
        <div className="relative center-row gap-2 w-full">
          <p className="w-52">
            Кол. товаров :{" "}
            <span className="font-bold">{data?.allProductsCount}</span>
          </p>
          <input
          ...
          <table className="table-fixed min-w-[1280px] w-full">
            <thead>
              <tr className="border-b border-support-200">
                ...
                <th className="w-16" onClick={() => toggleSortByField("order")}>
                  Номер
                  {renderArrows("order")}
                </th>
                ...
                <th
                  ...
                  onClick={() => toggleSortByField("stock")}
                >
                  Склад {renderArrows("stock")}
                </th>
                <th className="w-20">Ед. изм.</th>
                <th className="w-56">Подкатегория</th>
                <th
                  className="w-20"
                  onClick={() => toggleSortByField("createdAt")}
                >
                  Создано
            ...
            <tbody>
              {sortedRows?.map((row, index) => (
                ...
                >
                  ...
                  <td className="max-w-16">{row.order}</td>
                  ...
                  <td className="max-w-20">
                    {unitNamesRu[row.unit] || row.unit}
                  </td>
                  <td className="overflow-hidden max-w-56">
                    {row.SubCategory?.nameRu}
                  </td>
                  <td className="max-w-24 text-xs">{row.createdAt}</td>
