...
import OrderCitySelector from "../selectors/OrderCitySelectorO";
import PaymentTypeSelector from "../selectors/PaymentTypeO";
import DeliveryTypeSelector from "../selectors/DeliveryTypeSelectorO";
...
const fetchOrders = async (filters) => {
  const response = await fetch(`${apiUrl}/orders/delivered`, {
    method: "POST",
    body: JSON.stringify({
      page: filters.page || 1,
      limit: 20,
      query: filters?.query,
      orderCityId: Number(filters?.orderCityId) || null,
      orderTimeId: Number(filters?.orderTimeId) || null,
      orderStatusId: Number(filters?.orderStatusId) || null,
      paymentTypeId: Number(filters?.paymentTypeId) || null,
      deliveryTypeId: Number(filters?.deliveryTypeId) || null,
    ...
    <div className="flex flex-col gap-2 min-h-[85vh]">
      <div className="flex flex-col md:flex-row gap-2">
        <div className="center-row gap-1 w-full">
          <p className="min-w-32">Регион:</p>
          <OrderCitySelector
            value={localFilters.orderCityId}
            onChange={(value) => handleLocalFilterChange("orderCityId", value)}
          />
        </div>
        <div className="center-row gap-1 w-full">
          <p className="min-w-32">Оплата:</p>
          <PaymentTypeSelector
            value={localFilters.paymentTypeId}
            onChange={(value) =>
              handleLocalFilterChange("paymentTypeId", value)
            }
          />
        </div>
        <div className="center-row gap-1 w-full">
          <p className="min-w-32">Доставка:</p>
          <DeliveryTypeSelector
            value={localFilters.deliveryTypeId}
            onChange={(value) =>
              handleLocalFilterChange("deliveryTypeId", value)
            }
          />
        </div>
      </div>
      <div className="center-row gap-1 w-full">
        <input
          className="dark:bg-darkTwo basic-border-2 focus:border-primary outline-none dark:text-support px-2 h-9 md:h-10 w-full"
          type="text"
          placeholder="Поиск по:заказ, телефон, имя, улица"
      ...
      <div>
        {isLoading ? (
          <div className="bg-support border border-support-200 rounded center-col mt-2 h-28 w-full">
