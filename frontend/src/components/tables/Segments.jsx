...
import { Search, Image, ArrowUp, ArrowDown } from "lucide-react";
...
export default function SegmentsTable() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setData(response);
      setSortedRows(response?.segments);
    };
  ...
  return (
    <div className="flex flex-col">
      <div className="bg-support dark:bg-darkTwo basic-border rounded p-1 md:p-2 pb-4">
        <div className="relative center-row w-full">
          ...
          <button className="border-l border-support-200 rounded-r center-col absolute right-0 size-9 md:size-10">
            <Search className="text-dark dark:text-white hover:text-primary size-5" />
          </button>
        </div>
          ...
          <table className="table-fixed min-w-[600px] w-full">
            <thead>
              <tr className="border-b border-support-200">
                ...
                <th className="text-xs md:text-sm">Подкатегория</th>
            ...
            <tbody>
              {sortedRows?.map((row, index) => (
                <tr
                  ...
                  onClick={() => {
                    NProgress.start();
                    router.push(`/home/segments/${row.id}`);
                  }}
                ...
                >
                  <td className="bg-white dark:bg-dark rounded center-col p-1 h-12 w-16">
                    {row.image === "" ? (
                      <Image className="text-dark size-5" />
                    ) : (
                      <img
                        src={`${apiUrl}/${row.image}`}
                        width={112}
                        height={112}
                        alt="image of segment"
                  ...
                  <td className="text-left text-xs md:text-sm font-bold">
