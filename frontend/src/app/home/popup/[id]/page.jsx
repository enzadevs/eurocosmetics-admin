...
const fetchBannerData = async (id) => {
  const response = await fetch(`${apiUrl}/popup/fetch/${id}`);
...
export default function UpdateBannerPage({ params }) {
  ...
  const updateData = async (data) => {
    ...
    try {
      ...
      formData.append("countdown", data.countdown || bannerData?.countdown);
      ...
      const response = await fetch(
        `${apiUrl}/popup/update/${Number(params.id)}`,
        {
          method: "PATCH",
          body: formData,
        }
      ...
      if (response.ok) {
        SuccessToast({ successText: "PopUp обновлен." });

        newAction(
          admin?.user?.Role,
          admin?.user?.username,
          `Обновил PopUp с ID : ${params.id}`,
          "UPDATE"
        );
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`${apiUrl}/popup/delete/${params.id}`, {
        method: "DELETE",
      });

      if (response.ok) {
        SuccessToast({ successText: "PopUp удален." });
        setTimeout(() => {
          NProgress.start();
          router.push("/home/popup");
        }, 1000);
  ...
  const confirmDelete = (event) => {
    ...
    if (window.confirm(`Вы уверены, что хотите удалить PopUp?`)) {
      handleDelete();
    }
  };
  ...
  return (
    <div className="flex flex-col">
      <div className="center-row h-12">
        <BackForthButtons />
        <h2 className="ml-auto md:ml-0">Обновить PopUp</h2>
      </div>
